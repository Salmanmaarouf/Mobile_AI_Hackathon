...
public enum MaskAlgebra {
    ...
    ///
    /// Note that this inverts ALPHA too, which is what the segmentation path
    /// wants — it builds alpha-carrying mattes. Anything feeding a mask to
    /// `CIBlendWithMask`, which reads the mask's alpha, wants
    /// `invertKeepingAlpha` instead.
    ...
    /// Inverts a mask's VALUE — `rgb = 1 - rgb` — and leaves alpha opaque.
    ///
    /// For a mask destined for `CIBlendWithMask`, which reads the mask's ALPHA,
    /// plain `invert` is quietly destructive: `holeMask` forces alpha to 1 and
    /// carries its signal in RGB, so inverting alpha turns an opaque mask into
    /// a fully TRANSPARENT one — and a zero-alpha mask selects the background
    /// everywhere.
    ///
    /// That is what made `punchingHole` erase the whole plate rather than just
    /// the subject, leaving the push-pull after it with no surviving pixels to
    /// draw colour in from. Everything downstream of the fill came back black:
    /// the background layer, the outpainted backdrop, the ambient sphere.
    public static func invertKeepingAlpha(_ image: CIImage) -> CIImage {
        let f = CIFilter.colorMatrix()
        f.inputImage = image
        f.rVector = CIVector(x: -1, y: 0, z: 0, w: 0)
        f.gVector = CIVector(x: 0, y: -1, z: 0, w: 0)
